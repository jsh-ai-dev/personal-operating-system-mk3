...
배경: 사용자는 6년 차 개발자로, 처음 접하는 기술들을 배우고 있습니다.
...
절대 금지: 보기 텍스트에 "A.", "B.", "C.", "1.", "①" 등 번호·기호 접두사 포함 금지.
...
class ChatService:
    ...
    async def generate_quiz(self, owner_id: str, conversation_id: str, model: str) -> dict:
        ...
        # AI가 "A. 보기" 형태로 접두사를 붙이는 경우 제거
        _prefix = _re.compile(r"^[A-Da-d][.\)]\s*|^[①②③④]\s*|^\d+[.\)]\s*")
        for q in quiz:
            q["options"] = [_prefix.sub("", o) for o in q.get("options", [])]
